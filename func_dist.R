...
funcdist <- function(resnum, mutAA, deval, distances, function_name, weight_function, a){
  ...
  for(dists in names(distances[[resnum]])){
    for (flist in deval[deval$resnum==dists & deval$mutAA != mutAA, function_name]){
      if (!is.na(flist) & weight_function == "sigmoid"){
        sum_weights <- sum_weights + sigmoid(as.numeric(distances[[resnum]][dists]))
        sum_function <- sum_function + flist*sigmoid(as.numeric(distances[[resnum]][dists]))
      } else if (!is.na(flist) & weight_function == "sin") {
        sum_weights <- sum_weights + sin_dist(as.numeric(distances[[resnum]][dists]),a)
        sum_function <- sum_function + flist*sin_dist(as.numeric(distances[[resnum]][dists]),a)
      }
    }
  }
